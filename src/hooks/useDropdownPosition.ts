...
export interface DropdownPosition {
    ...
    /** false until the first position calculation completes */
    ready: boolean;
}
...
export function useDropdownPosition(
    ...
    const calculatePosition = useCallback(() => {
        ...
        setPosition({ top, left, maxHeight, transformOrigin, ready: true });
    }, [anchorRef, dropdownWidth, dropdownHeight, gap]);

    useEffect(() => {
        if (!isOpen) {
            // Reset ready so next open waits for position calculation
            setPosition(prev => prev.ready ? { ...prev, ready: false } : prev);
            return;
        }

        // Wait one frame for anchor layout to settle before calculating
